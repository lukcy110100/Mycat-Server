 ...
 * @author mycat
 */
public final class SelectSchemaColumns
{
    private static final Logger LOGGER = LoggerFactory.getLogger(SelectSchemaColumns.class);


    public static void execute(ServerConnection c, String sql) {


    List<String>  splitVar= Lists.newArrayList( "COLUMN_NAME","CHARACTER_SET_NAME","COLLATION_NAME","COLUMN_TYPE") ;
...
//        }
        ...
        for (String name : values) {
            ...
            row.add(StringUtil.encode("utf8", c.getCharset()));
            row.add(StringUtil.encode("utf8_general_ci", c.getCharset()));
            row.add(StringUtil.encode("char(255)", c.getCharset()));
            row.packetId = ++packetId;
            buffer = row.write(buffer, c,true);
        }
